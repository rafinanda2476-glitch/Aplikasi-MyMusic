...
# -------------------------
ALLOWED_GENRES = [
    "Jazz","Blues","RnB","Rock","Classic",
    "EDM (Electronic Dance Music)","Reggae","Hip Hop","Pop"
]
library = SongLibrary()
...
# spotify_gui_final.py - PART 2
# LANJUTAN DARI PART 1 - Copy semua code dari Part 1, lalu tambahkan code di bawah ini
...
class App:
    ...
    def show_admin_main(self):
        ...
        Button(topbar, text="Import CSV", command=lambda: self.safe_switch(self.show_admin_import_csv)).pack(side=LEFT, padx=6)
    ...
    def show_admin_import_csv(self):
        for w in self.content.winfo_children(): w.destroy()
        Label(self.content, text="Import Lagu dari CSV", font=("Arial", 14, "bold")).pack(anchor=W, pady=6)
    
        self.selected_csv_path = None
        path_label = Label(self.content, text="Belum ada file dipilih", fg="blue")
        path_label.pack(anchor=W, pady=6)
        
        preview_frame = Frame(self.content)
        preview_frame.pack(fill=BOTH, expand=True, pady=6)
        preview_scroll = Scrollbar(preview_frame)
        preview_scroll.pack(side=RIGHT, fill=Y)
        preview_box = Listbox(preview_frame, width=110, height=15, yscrollcommand=preview_scroll.set)
        ...
        def browse_csv():
            ...
            if filepath:
                self.selected_csv_path = filepath
                path_label.config(text=f"File: {filepath}")
            ...
            try:
                with open(filepath, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        ...
                        preview_box.insert(END, f"{title} - {artist} ({genre}, {year})")
                        count += 1
                        if count >= 50:  # limit preview
                            preview_box.insert(END, "... (dan lainnya)")
                            break
                if count == 0:
                    preview_box.insert(END, "(File CSV kosong)")
            except Exception as e:
                preview_box.insert(END, f"Error membaca file: {e}")
                messagebox.showerror("Error", f"Gagal membaca CSV: {e}", parent=self.root)
        
        def import_csv():
            if not self.selected_csv_path:
                messagebox.showerror("Error", "Pilih file CSV terlebih dahulu!", parent=self.root)
                return
            ...
            try:
                ...
                with open(self.selected_csv_path, 'r', encoding='utf-8') as f:
                    reader = csv.DictReader(f)
                    ...
                    for idx, row in enumerate(reader, start=1):
                        try:
                            ...
                            if not title or not artist or not genre or not year_text:
                                skipped += 1
                                errors.append(f"Baris {idx}: Field tidak lengkap")
                                continue
                            
                            if genre not in ALLOWED_GENRES:
                                skipped += 1
                                errors.append(f"Baris {idx}: Genre '{genre}' tidak valid")
                                continue
                            ...
                            try:
                                ...
                                skipped += 1
                                errors.append(f"Baris {idx}: Tahun '{year_text}' bukan angka")
                                continue
                ...
                # Show result
                result_msg = f"Import selesai!\n\nBerhasil: {imported} lagu\nDi skip: {skipped} baris"
                if errors and len(errors) <= 10:
                    result_msg += "\n\nError:\n" + "\n".join(errors)
                elif errors:
                    result_msg += f"\n\nError: {len(errors)} baris (terlalu banyak untuk ditampilkan)"
                
                messagebox.showinfo("Import Selesai", result_msg, parent=self.root)
                
                if imported > 0:
                    self.show_admin_list()
                    
            except Exception as e:
                messagebox.showerror("Error", f"Gagal mengimpor CSV: {e}", parent=self.root)
        
        btn_frame = Frame(self.content)
        btn_frame.pack(anchor=W, pady=8)
        Button(btn_frame, text="Pilih File CSV", command=browse_csv).pack(side=LEFT, padx=6)
        Button(btn_frame, text="Import", command=import_csv).pack(side=LEFT, padx=6)
        Button(btn_frame, text="Batal", command=lambda: self.safe_switch(self.show_admin_main)).pack(side=LEFT, padx=6)
    ...
    def show_edit_form(self, song_id):
        ...
        # spotify_gui_final.py - PART 3 (FINAL)
